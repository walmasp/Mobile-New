...
class EspressoExtractorScreen extends StatefulWidget {
  ...
  @override
  State<EspressoExtractorScreen> createState() => _EspressoExtractorScreenState();
}
    ...
    showDialog(
      ...
      builder: (context) {
        return AlertDialog(
          ...
          title: const Text("✨ LUAR BIASA! ✨", textAlign: TextAlign.center, style: TextStyle(color: Colors.brown)),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              Container(
                padding: const EdgeInsets.all(15),
                decoration: BoxDecoration(color: Colors.brown[200]?.withOpacity(0.5), shape: BoxShape.circle),
                child: const Icon(Icons.coffee, size: 60, color: Colors.brown),
              ),
              const SizedBox(height: 15),
              const Text(
                "Jarimu sangat lincah!\n\nKamu berhasil mengekstrak Espresso tanpa membuat mesin overheat. Klaim Free Upsize di kasir ya!",
          ...
          actions: [
            Center(
              child: ElevatedButton(
                ...
                child: const Text("Tutup & Klaim", style: TextStyle(color: Colors.white)),
              ),
            ),
          ],
        );
            ...
            children: [
              Container(
                ...
                child: Text(
                  _status,
                  textAlign: TextAlign.center,
                  style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: _isGameOver ? Colors.red : Colors.brown[800]),
                ),
              ),
              const SizedBox(height: 40),
              const Align(
                alignment: Alignment.centerLeft,
                child: Text("Hasil Ekstraksi Kopi:", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.brown)),
              ),
              ...
              Text("${_coffeeExtracted.toInt()}%", style: const TextStyle(fontWeight: FontWeight.bold)),
              const SizedBox(height: 30),
              const Align(
                alignment: Alignment.centerLeft,
                child: Text("Suhu Mesin (Jangan sampai penuh!):", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.brown)),
              ),
              const SizedBox(height: 5),
              ClipRRect(
                borderRadius: BorderRadius.circular(20),
              ...
              Text("${_heatLevel.toInt()}°C", style: TextStyle(fontWeight: FontWeight.bold, color: heatColor)),
              const SizedBox(height: 60),
              Transform.translate(
                offset: Offset((_heatLevel > 80 && _isNear) ? (_heatLevel % 3 - 1.5) * 2 : 0, 0),
                child: Icon(Icons.coffee_maker_rounded, size: 100, color: _heatLevel > 80 ? Colors.red : Colors.brown[700]),
              ),
              const SizedBox(height: 40),
              if (!_isPlaying)
                ElevatedButton(
                  ...
                  child: Text(
                    _isGameOver || _coffeeExtracted >= 100 ? "Main Lagi" : "Mulai Ekstraksi",
